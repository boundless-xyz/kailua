...
- [ ] 2.1 Create `crates/kona/src/hash/mod.rs` module. Define a canonical state normalization trait/function that accepts BOTH revm's `Cache` (from `CacheDB`, uses `DbAccount`/`AccountState`) and `CacheState` + `State.block_hashes` (from `State`, uses `CacheAccount`/`AccountStatus`) and produces an identical byte encoding for the same logical state. The normalization maps `AccountStatus` → `AccountState` equivalence for HASHING ONLY (Loaded→None, Changed→Touched, Destroyed→StorageCleared, InMemoryChange→Touched, etc.) and extracts nonce/balance/code_hash/storage from both `PlainAccount` and `DbAccount` uniformly.
- [ ] 2.2 Implement `hash_canonical_state(...) -> B256` that takes the normalized representation and computes deterministic SHA256. Canonical encoding: accounts (sorted by Address) with full account info + storage (sorted by U256) + normalized account state, then contracts (sorted by code_hash) with bytecode, then block_hashes (sorted by U256 block number). Explicitly EXCLUDE `Cache.logs` from this hash, since logs are committed by the EVM accumulator hash via receipts / logs bloom.
