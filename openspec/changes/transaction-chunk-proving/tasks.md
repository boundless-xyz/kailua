...
- [ ] 2.1 Create `crates/kona/src/hash/mod.rs` module. Define a canonical state normalization trait/function that accepts BOTH revm's `Cache` (from `CacheDB`, uses `DbAccount`/`AccountState`) and `CacheState` (from `State`, uses `CacheAccount`/`AccountStatus`) and produces an identical byte encoding for the same logical state. The normalization maps `AccountStatus` → `AccountState` equivalence (Loaded→None, Changed→Touched, Destroyed→StorageCleared, InMemoryChange→Touched, etc.) and extracts nonce/balance/code_hash/storage from both `PlainAccount` and `DbAccount` uniformly.
- [ ] 2.2 Implement `hash_canonical_state(...) -> B256` that takes the normalized representation and computes deterministic SHA256. Canonical encoding: accounts (sorted by Address) with full account info + storage (sorted by U256) + normalized account state, then contracts (sorted by code_hash) with bytecode, then block_hashes (sorted by U256 block number).
- [ ] 2.3 Add unit tests: hash a `Cache` and a `CacheState` that represent the same logical state → verify identical hashes. Empty state, single account, multiple accounts with storage, contracts present, block_hashes present. Verify any field change (nonce, balance, code_hash, storage slot, account_state, contract bytecode, block_hash) produces a different hash. Verify insertion order independence.
- [ ] 2.4 Create `hash_evm_state(evm_state: &EvmAccumulatorState) -> B256` function in the same module. Define `EvmAccumulatorState` struct with `cumulative_gas_used: u64`, `da_footprint_used: u64`, `blob_gas_used: u64`, `logs_bloom: Bloom`, `receipts: Vec<OpReceiptEnvelope>`. Hash with SHA256 over canonical encoding.
- [ ] 2.5 Add unit tests for `hash_evm_state`: initial (zeroed) state, state with receipts, modification of any field changes the hash, receipt ordering matters.
- [ ] 2.6 Create `compute_chunk_trace(tx_hash: B256, pre_db_hash: B256, post_db_hash: B256, pre_evm_hash: B256, post_evm_hash: B256) -> B256` function. Returns `SHA256(tx_hash || pre_db_hash || post_db_hash || pre_evm_hash || post_evm_hash)`.
- [ ] 2.7 Add unit tests for `compute_chunk_trace`: determinism, any input change produces different output, integration with `Precondition::default().chunk(trace).digest()`.
- [ ] 2.8 Run full `kailua-kona` test suite — verify zero regressions.

## 3. TracingOpEvm and TracingEvmFactory

- [ ] 3.1 Create `crates/kona/src/tracing_evm.rs`. Define `TracingOpEvm<DB, I>` struct wrapping `OpEvm<DB, I, PrecompilesMap>` with `traces: Arc<Mutex<Vec<EvmState>>>`. Implement `Deref<Target = OpContext<DB>>` via `self.inner.ctx()` and `DerefMut` via `self.inner.ctx_mut()` — this is required because `OpBlockExecutor` accesses EVM fields through Deref, not the Evm trait. Implement all 8 required `Evm` trait methods: `block()`, `chain_id()`, `finish()`, `set_inspector_enabled()`, `components()`, `components_mut()` delegate directly to inner. `transact_raw()` delegates then clones `result.state` into traces on success. `transact_system_call()` delegates transparently without appending to the tx-body trace buffer. Note: `db()`, `db_mut()`, and 8 other methods are provided defaults on the Evm trait and do NOT need manual implementation.
- [ ] 3.2 Define associated types on `TracingOpEvm`: `type DB = DB`, `type Tx = OpTransaction<TxEnv>`, `type Error = EVMError<DB::Error, OpTransactionError>`, `type HaltReason = OpHaltReason`, `type Spec = OpSpecId`, `type BlockEnv = BlockEnv`, `type Precompiles = PrecompilesMap`, `type Inspector = I`. These must exactly match `OpEvm`'s associated types.
- [ ] 3.3 Define `TracingEvmFactory` struct with `traces: Arc<Mutex<Vec<EvmState>>>`. Implement `EvmFactory` with `type Evm<DB, I> = TracingOpEvm<DB, I>` and matching associated types: `type Context<DB> = OpContext<DB>`, `type Tx = OpTransaction<TxEnv>`, `type Error<DBError> = EVMError<DBError, OpTransactionError>`, `type HaltReason = OpHaltReason`, `type Spec = OpSpecId`, `type BlockEnv = BlockEnv`, `type Precompiles = PrecompilesMap`. Both `create_evm` and `create_evm_with_inspector` delegate to `OpEvmFactory` then wrap the result. The factory must satisfy `EvmFactory<Spec = OpSpecId, BlockEnv = BlockEnv> + Send + Sync + Clone + 'static` to match `KonaExecutor`'s `Executor` impl bounds.
- [ ] 3.4 Add unit tests for `TracingEvmFactory`: construct factory, create EVM, execute a simple transaction against an `InMemoryDB`, verify trace buffer contains exactly one entry with the expected touched addresses/slots. Test multiple transactions produce ordered tx-body trace entries. Test block-level `transact_system_call()` delegation does not append an extra trace entry. Test error propagation (failed tx → no trace entry). Test that Deref-based field access (`.block()`, `.cfg`) works correctly on `TracingOpEvm`.
...
- [ ] 5.1 Create `crates/kona/src/chunk/mod.rs` module. Define rkyv-serializable mirror types for revm's `Cache` structure: `SerializableCache` (with `SerializableDbAccount`, serializable `AccountInfo`, `Bytecode`, `AccountState`). Implement `From<Cache> for SerializableCache` and `From<SerializableCache> for Cache` conversions. Add round-trip tests. Then define `ChunkWitness` struct using the serializable types: `block_number: u64`, `chunk_index: u16`, `total_chunks: u16`, `tx_start: u16`, `tx_count: u16`, `cache: SerializableCache` (chunk-start flat state after the block prelude), `evm_state: EvmAccumulatorState` (pre-chunk tx-body accumulators), `agreed_l2_output_root: B256`, `config_hash: B256`, `fpvm_image_id: B256`, `payout_recipient: Address`.
...
- [ ] 6.1 Create `crates/kona/src/chunk/prove.rs`. Implement `run_chunk_proof(witness: ChunkWitness) -> ProofJournal` that: (a) converts `SerializableCache` → `Cache`, builds `CacheDB<PanicDB>` by setting `cache_db.cache = cache`, (b) computes pre_db_hash using canonical hash, (c) sets up EVM environment from witness block context, (d) wraps `CacheDB<PanicDB>` in `State::builder().with_database(...).with_bundle_update().without_state_clear().build()`, creates EVM and executor via `OpBlockExecutorFactory`, (e) does NOT call `apply_pre_execution_changes()` — prelude is handled by the aggregation proof, (f) executes only the chunk's ordered transaction body, (g) computes post_db_hash and post_evm_state_hash, (h) computes chunk_trace, (i) returns ProofJournal with chunk sentinel. Note: `EmptyDB` returns defaults (not panics), so `PanicDB` is required.
...
- [ ] 7.2 Implement the aggregation flow in `verify_and_aggregate_chunks(...)`:
  ...
  (b) Call `executor.apply_pre_execution_changes()` — prelude runs (beacon root, blockhashes, Canyon deployer). Optionally call `executor.apply_post_execution_changes()` for forward compatibility.
  ...
  (g) Apply epilogue manually: `post_block_balance_increments()` (public fn from `alloy_evm::block::state_changes`) → `state.increment_balances()` (public on `State<DB>`).
  (h) `state.merge_transitions(BundleRetention::Reverts)` → `state.take_bundle()` → `trie_db.state_root(&bundle)`.
  (i) Build header manually: `compute_receipts_root()` (public), `ordered_trie_with_encoder()` (public), `logs_bloom()` (public). Replicate EIP-1559 `extra_data` encoding (~40 lines from `pub(crate)` kona-executor utils). Construct `BlockBuildingOutcome`.
- [ ] 7.3 Add unit tests for `verify_and_aggregate_chunks`: single chunk (trivial aggregation), two chunks with correct hash chain, hash mismatch between chunks causes failure, post-transaction cache hash mismatch causes failure, final evm state hash mismatch causes failure, prelude is applied exactly once, epilogue is applied exactly once. Verify the produced `BlockBuildingOutcome` (state_root, receipts_root, gas_used) matches monolithic execution.
