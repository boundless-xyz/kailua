...
### Requirement: Chunk guest executes transactions against CacheDB with PanicDB fallback

The chunk proving mode SHALL execute a subset of a block's transactions against a `CacheDB<PanicDB>` (or equivalent in-memory DB backed by a panicking fallback). All required state MUST be pre-loaded in the cache. If any transaction reads state not present in the cache, the guest SHALL panic.
...
#### Scenario: pre_db_hash reflects loaded state
...
- **THEN** `pre_evm_state_hash` is computed from the initial (zeroed) EVM accumulators
