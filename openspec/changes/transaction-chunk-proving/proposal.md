...
- **Chunk proving mode**: A guest execution mode that operates on a flat in-memory state (`CacheDB<PanicDB>`) rather than a full state trie. Executes a configured subset of a block's transactions and emits a standard `ProofJournal` with `l1_head = 0xFF..FF` as the chunk sentinel.
- **Block aggregation mode**: A CachedExecutor path that verifies chunk proof receipts, chains memory DB and EVM state hashes for continuity, loads the final state from the witness, diffs against the initial trie state, and produces a standard `BlockBuildingOutcome` — identical to monolithic execution output.
- **Chunk witness construction**: Host-side logic that uses per-transaction traces to compute minimal flat state witnesses for each chunk, accounting for intra-block state dependencies between chunks.
